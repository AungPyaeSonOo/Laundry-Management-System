...
const PackerHistory = () => {
  ...
  const [stats, setStats] = useState({
    incoming: 0,      // ironing (ဝင်လာတာ)
    in_progress: 0,   // ready (လုပ်နေတာ)
    completed: 0      // delivered (ပြီးသွားတာ - Delivery ဆီပို့)
  });
  ...
  const fetchHistory = async () => {
    try {
      ...
      // ✅ Packer နဲ့ဆိုင်တဲ့ Status တွေပဲယူ
      const response = await orderApi.getAll({ 
        ...params,
        status: ['ironing', 'ready', 'delivered'],
        limit: 200
      });
      ...
      setStats({
        incoming: allOrders.filter(o => o.status === 'ironing').length,
        in_progress: allOrders.filter(o => o.status === 'ready').length,
        completed: allOrders.filter(o => o.status === 'delivered').length
      });

    } catch (error) {
      console.error('Error fetching packer history:', error);
    ...
    <div className="animate-fade-in">
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-4">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h4 className="fw-bold mb-1">📊 Packer History</h4>
            <p className="text-secondary mb-0">View your packing history</p>
          </div>
        </div>
      </div>
        ...
        <Col xs={6} md={4}>
          <Card className="shadow-sm border-0 h-100">
            <Card.Body className="text-center p-3">
              ...
              <div className="text-secondary small">Incoming (From Ironer)</div>
