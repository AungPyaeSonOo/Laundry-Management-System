...
const IronerOrderDetail = () => {
  ...
  const fetchOrder = async () => {
    try {
      ...
      navigate('/ironer-dashboard');
    } finally {
      setLoading(false);
    }
  };

  const handleStartIroning = async () => {
    if (!window.confirm('Start ironing this order?')) {
      return;
    }
    ...
    try {
      await orderApi.completeNext(order.order_id, { 
        notes: notes || 'Started ironing' 
      });
      toast.success('✅ Ironing started!');
      navigate('/ironer-dashboard');
    } catch (error) {
      toast.error('Failed to start ironing');
    } finally {
      setProcessing(false);
    }
  };

  const handleCompleteIroning = async () => {
    if (!window.confirm('Complete ironing and send to Packer?')) {
      return;
    }
    ...
    try {
      await orderApi.completeNext(order.order_id, { 
        notes: notes || 'Ironing completed, sent to Packer' 
      });
      toast.success('✅ Ironing completed! Sent to Packer.');
      navigate('/ironer-dashboard');
    } catch (error) {
      toast.error('Failed to complete ironing');
    } finally {
      setProcessing(false);
    }
  };
            ...
            {isWashing && (
              <Col xs={12}>
                <Button 
                  ...
                  onClick={handleStartIroning}
                  disabled={processing}
                >
                  {processing ? (
                    <Spinner animation="border" size="sm" />
                  ) : (
                    <><FiWind className="me-2" /> Start Ironing</>
                  )}
                </Button>
              </Col>
            )}
            ...
            {isIroning && (
              <Col xs={12}>
                <Button 
                  ...
                  size="lg"
                  className="w-100 py-3"
                  onClick={handleCompleteIroning}
                  disabled={processing}
                >
                  {processing ? (
                    <Spinner animation="border" size="sm" />
                  ) : (
                    <><FiCheckCircle className="me-2" /> Complete Ironing & Send to Packer</>
                  )}
                </Button>
              </Col>
            )}
...
export default IronerOrderDetail;